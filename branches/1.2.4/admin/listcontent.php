...
#
#$Id: listcontent.php 4158 2007-09-16 18:57:04Z wishy $
...
function display_hierarchy(&$root, &$userid, $modifyall, &$templates, &$users, &$menupos, &$openedArray, &$pagelist, &$image_true, &$image_set_false, &$image_set_true, &$upImg, &$downImg, &$viewImg, &$editImg, &$deleteImg, &$expandImg, &$contractImg, &$mypages, &$page)
{
    ...
    if ($display != 'none')
    {
		...
		if (check_modify_all($userid))
		{
			...
			{
				$thelist .= "<td class=\"pagepos\">".($one->DefaultContent()?$image_true:"<a href=\"listcontent.php?makedefault=".$one->Id()."\" onclick=\"if(confirm('".lang("confirmdefault", htmlentities($one->mName))."')) xajax_content_setdefault(".$one->Id().");return false;\">".$image_set_true."</a>")."</td>\n";
			}
			else
			{
				$thelist .= "<td>&nbsp;</td>";
        ...
        if ($display == 'edit' || $display == 'structure')
        {
            ...
            if ($one->DefaultContent() != true)
            {
                ...
                {
                    $thelist .= "<td class=\"pagepos\"><a href=\"listcontent.php?deletecontent=".$one->Id()."\" onclick=\"if (confirm('".lang('deleteconfirm', htmlentities($one->mName))."')) xajax_content_delete(".$one->Id()."); return false;\">";
                    $thelist .= $deleteImg;
                    $thelist .= "</a></td>\n";
                }
                else
