...
function smarty_function_syntax_area($params, &$template)
{
	...
	if( !isset($params['name']) ) throw new CmsInvalidDataExtension('syntax_area misssing parameter: name');

	$name = trim($params['name']);
	if( isset($params['prefix']) ) $name = trim($params['prefix']).$name;

	$value = '';
	if( isset($params['value']) ) $value = trim($params['value']);
	...
	$rows = 15;
	$cols = 80;
	if( isset($params['rows']) ) $rows = (int)$params['rows'];
	if( isset($params['cols']) ) $cols = (int)$params['cols'];
