  ...
  if ($originator == 'Core')
	...
	case 'EditTemplatePost':
	  ...
	  if( $template->active != false ) {
	    ...
	    $q = "SELECT id FROM ".cms_db_prefix()."module_search_items WHERE content_id = ? AND extra_attr = ?";
	    ...
	    if( ! $non_indexable ) {
	      ...
	      $module->AddWords($module->GetName(), $template->id, 'template', $content);
	    }
	    else {
	      $module->DeleteWords($module->GetName(), $template->id, 'template');
	    }
	    ...
	    if( ($non_indexable && $was_indexed) ) {
	      ...
	      // the children.
	      $q2 = "DELETE FROM ".cms_db_prefix()."module_search_items WHERE extra_attr = ? AND content_id  IN (";
	      $parms = array( 'content' );
