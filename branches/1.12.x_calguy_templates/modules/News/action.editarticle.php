...
$news_url = '';
if (isset($params['news_url']))	$news_url = $params['news_url'];
...
$usedcategory = '';
if (isset($params['category'])) $usedcategory = $params['category'];

$author_id = '-1';
if (isset($params['author_id'])) $author_id = $params['author_id'];
...
if( isset($params['submit']) || isset($params['apply']) ) {
  ...
  if( empty($title) ) {
    ...
    if( $startdate >= $enddate ) $error = $this->Lang('error_invaliddates');
  }
  ...
  if( !$error ) {
	...
	if( $tmp === false ) {
	  if( !empty($value) ) {
	    ...
	    $dbr = $db->Execute($query,	array($articleid,$fldid,$value));
	  }
	}
	else {
	  if( empty($value) ) {
...
if( $author_id > 0 ) {
  ...
  if( $feu ) {
    ...
    if( $uinfo[0] ) $smarty->assign('inputauthor',$uinfo[1]['username']);
  }
}
