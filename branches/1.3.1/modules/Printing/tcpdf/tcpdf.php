...
if(!class_exists('TCPDF')) {
	 ...
	 */ 
	define('PDF_PRODUCER','TCPDF 3.0.012_PHP4 (http://www.tcpdf.org)');
			...
			if ($arabic) {
				...
				for ($i=0; $i < $numchars; $i++) {
					...
					if ($unicode[$thischar['char']] == 'AL') {
						...
						// if laa letter
						if (($prevchar['char'] == 1604) AND (in_array($thischar['char'], $alfletter))) {
							$arabicarr = $laa_array;
							$laaletter = true;
							$prevchar  = $charAL[($x-2)];
						} else {
						...
						if (($i > 0) AND (($i+1) < $numchars) AND
							...
							if (($thischar['char'] == 1607) AND
								...
								// Allah Word
								$chardata2[$i-2]['char'] = $chardata2[$i-1]['char'] = "0";
								$chardata2[$i-2]['type'] = $chardata2[$i-1]['type'] = 'B';
						...
						// if laa letter
						if($laaletter) {
							$chardata2[($charAL[($x-1)]['i'])]['char'] = "0";
							$chardata2[($charAL[($x-1)]['i'])]['type'] = 'B';
						}
					}
				}
				// move adjacent NSM characters to avoid overlapping
				/*
				for ($i=1; $i < $numchars; $i++) {
					if (($unicode[$chardata2[$i]['char']] == 'NSM') AND 
						($unicode[$chardata2[$i+1]['char']] == 'NSM')) {
						if (($i <= 1) OR (($i > 1) AND ($unicode[$chardata2[$i-2]['char']] != 'NSM'))) {
							// swap NSM character with previous
							$tmpchr = $chardata2[$i-1];
							$chardata2[$i-1] = $chardata2[$i];
							$chardata2[$i] = $tmpchr;
						} elseif ($i > 1) {
								if ((($i+3) == $numchars) OR ((($i+3) < $numchars) AND ($unicode[$chardata2[$i+3]['char']] != 'NSM'))) {
								// swap NSM character with next
								$tmpchr = $chardata2[$i+2];
								$chardata2[$i+2] = $chardata2[$i+1];
								$chardata2[$i+1] = $tmpchr;
							}
						}
					}
				}
