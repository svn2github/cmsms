...
for ($i=0; $i<count($filelist);$i++) {
  ...
  if ($filelist[$i]["name"]!="..") {
		if ($filelist[$i]["writable"]) {			
          ...
          $onerow->chmodaction=$this->CreateLink($id,"chmoddir",'',$this->GetActionIcon("chmod"),array("filename"=>$filelist[$i]["name"],"path"=>$path,"sortby"=>$sortby),"",false,false,"title='".$this->Lang("chmod")."'");          
  		  }
