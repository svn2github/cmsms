...
if (isset($params["newname"]) && !empty($params["newname"])) {
	...
	if (isset($params["cancel"])) {
		$this->Redirect($id,"defaultadmin",$returnid,array("path"=>$params["path"],"fmmessage"=>"renamecancelled"));
	} else {
		if (rename($fullname,$fullnewname)) {
			$this->Redirect($id,"defaultadmin",$returnid,array("path"=>$params["path"],"fmmessage"=>"renamesuccess"));
		} else {
			$this->Redirect($id,"defaultadmin",$returnid,array("path"=>$params["path"],"fmerror"=>"renamefailure"));
		}
	}
} else {
