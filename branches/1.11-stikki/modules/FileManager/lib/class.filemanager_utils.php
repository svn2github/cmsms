...
final class filemanager_utils
{
  ...
  public static function check_advanced_mode()
  {
    ...
    $b = $filemod->GetPreference('advancedmode');
    if( $a && $b ) return TRUE;
    return FALSE;
  }
  ...
  {
    $config = cmsms()->GetConfig();
    $default = basename($config['uploads_path']);
    if( self::check_advanced_mode() )
      {
	// we can do advanced mode.
	$default = '/';
      }
    return $default;
  }
  ...
  public static function test_invalid_path($path)
  {
    ...
    if( !startswith($path,$config['root_path']) )
      {
	$path = filemanager_utils::join_path($config['root_path'],$path);
      }
    $path=realpath($path);
    if( $path === FALSE ) {
      return true;
  ...
  public static function get_cwd()
  {
    ...
    if( self::test_invalid_path($path) )
      {
	$config = cmsms()->GetConfig();
	$path = basename($config['uploads_path']);
      }
    return $path;
  }
  ...
  public static function set_cwd($path)
  {
    ...
    $tmp = cms_join_path($config['root_path'],$path);
    if( !is_dir($tmp) )
      {
  ...
  public static function join_path()
  {
    ...
    for( $i = 0; $i < count($args); $i++ )
      {
	if( $i != 0 && startswith($args[$i],'/') ) $args[$i] = substr($args[$i],1);
	if( endswith($args[$i],'/') ) $args[$i] = substr($args[$i],0,-1);
      }

    return call_user_func_array('cms_join_path',$args);
  }
  ...
  public static function get_file_list($path = '')
  {
	...
	$info['url']=$config['root_url'].self::join_path($path,$file);
