...
if ($access) {
	if (isset($_POST["editplugin"])) {
		...
		if ($validinfo) {
			...
			if ($result) {
				audit($_SESSION["cms_admin_user_id"], $_SESSION["cms_admin_username"], $userplugin_id, $plugin_name, 'Edited User Tag');
				redirect("plugins.php");
				return;
			}
			else {
...
<table border="0">
	...
	<tr>
		<td>&nbsp;</td>
		<td><input type="hidden" name="userplugin_id" value="<?=$userplugin_id?>" /><input type="hidden" name="editplugin" value="true" /><input type="submit" value="<?=$gettext->gettext("Submit")?>" /><input type="submit" name="cancel" value="<?=$gettext->gettext("Cancel")?>" /></td>
	</tr>
