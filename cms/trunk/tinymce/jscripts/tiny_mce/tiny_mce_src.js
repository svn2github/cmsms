...
function TinyMCE() {
	...
	this.cleanupHTML = TinyMCE_cleanupHTML;
...
function TinyMCE_updateContent(form_element_name) {
	...
	for (var instanceName in tinyMCE.instances) {
		...
		if (instance.formElement == formElement) {
			instance.contentWindow.document.body.innerHTML = instance.formElement.value;
...
function TinyMCE_triggerSave() {
	...
	for (var instanceName in tinyMCE.instances) {
		...
		var cleanedHTML = tinyMCE.cleanupHTML(tinyMCE.settings, instance.contentWindow.document.body, false, true);
...
function TinyMCE_setupContent(editor_id) {
	...
	if (tinyMCE.isMSIE) {
		...
		document.frames[editor_id].document.body.onblur = function() {
			var event = document.frames[editor_id].event;
	...
	if (!tinyMCE.isMSIE) {
		...
		instance.contentWindow.document.body.innerHTML = tinyMCE.cleanupHTML(this.settings, contentElement);
	} else {
	...
	if (tinyMCE.settings['handleNodeChangeCallback'])
		window.setTimeout(tinyMCE.settings['handleNodeChangeCallback'] + '("' + editor_id + '", tinyMCE.instances["' + editor_id + '"].contentWindow.document.body);', 1);
...
function TinyMCE_cleanupHTML(config, element, visual, on_save) {
...
function TinyMCE_getContent() {
	if (tinyMCE.selectedInstance) {
		var cleanedHTML = tinyMCE.cleanupHTML(tinyMCE.settings, this.selectedInstance.contentWindow.document.body, false, true);
		return cleanedHTML;
	}
...
function TinyMCE_setContent(html_content) {
	if (tinyMCE.selectedInstance) {
		this.selectedInstance.contentWindow.document.body.innerHTML = html_content;
...
function TinyMCEControl_execCommand(command, user_interface, value) {
	...
	switch (command) {
		...
		case "mceCleanup":
			var cleanedHTML = tinyMCE.cleanupHTML(this.settings, this.contentDocument.body);
...
/*
		case "mceSetStyle":
			...
			var elementName = value['element_name'];
			var elementAttribs = value['element_attribs'];
			var replaceElements = value['replace_elements'];
			...
			if (selectedText) {
				...
				elementArray = tinyMCE.getElementsByAttributeValue(this.contentDocument.body, "a", "href", "#mce_temp_url#");
			} else
				elementArray[elementArray.length] = tinyMCE.getParentElement(this.getFocusElement(), replaceElements);
			...
			for (var x=0; x<elementArray.length; x++) {
				...
				var styleElm = this.contentDocument.createElement(elementName);

				// Set element attributes
				if (elementAttribs) {
					for (var key in elementAttribs) {
						if (key == "class")
							styleElm.className = elementAttribs[key];
						else
							styleElm.setAttribute(key, elementAttribs[key]);
					}
				}
				...
				if (elm.hasChildNodes()) {
					for (var i=0; i<elm.childNodes.length; i++)
						styleElm.appendChild(elm.childNodes[i].cloneNode(true));
				}

				elm.parentNode.replaceChild(styleElm, elm);
			}
		break;
*/
		case "mceSetCSSClass":
