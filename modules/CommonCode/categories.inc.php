...
*/
function commoncode_module_get_categories($db, $categories_table_name, $order_by='category_order, category_name')
{
...
*/
function commoncode_module_admin_display_category($cms, $module_id, $module_name, $categories_table_name)
{
	...
	echo cms_mapi_create_admin_form_start($module_name, $module_id);
    echo <<<EOT
...
*/
function commoncode_module_admin_update_categories($cms, $module_id, $categories_table_name,
	...
	for($i = 0; $i < $num_records; $i++)
	{
		...
		if($id > -1)
		{
			if($name == '')
			{
				...
				$sql = "DELETE FROM $data_to_categories_table_name WHERE category_id = $id";
				$db->Execute($sql);
