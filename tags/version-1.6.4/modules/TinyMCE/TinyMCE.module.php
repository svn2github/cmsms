  ...
  function HasCapability($capability, $params=array()) {
	...
	function ResetSettings($whatsettings="all") {
      ...
      $this->SetPreference('advanced_toolbar1',"cut,paste,pastetext,pasteword,copy,separator,justifyleft,justifycenter,justifyright,justifyfull,separator,styleselect,formatselect,fontselect,fontsizeselect");
			$this->SetPreference('advanced_toolbar2',"bold,italic,underline,strikethrough,advhr,separator,bullist,numlist,separator,outdent,indent,separator,undo,redo,separator,customdropdown,cmslinker,link,unlink,anchor,image,charmap,cleanup,separator,forecolor,backcolor,separator,code,fullscreen,help");
...
Insert Smarty {literal} around selection|{literal}|{/literal}");
    ...
    //if ($frontend) echo "hi";
		...
		if ($this->wysiwygactive) {
			...
			if ($this->GetPreference("usestaticconfig")!="1") {
        $front="";
        if ($frontend) $front="&frontend=yes";
				$output.='
			  <script type="text/javascript" src="'.$gCms->config['root_url'].'/modules/TinyMCE/tinyconfig.php?templateid='.$this->templateid.'&amp;languageid='.$languageid.$front.'"></script>
        ';
			} else {
...
";
	...
	if ($this->GetPreference("cmslinkerstyle","selflink")=="a") {
      ...
      if ($config['url_rewriting'] == 'mod_rewrite') {
  		  $link = $config['root_url']."/".$entry->mHierarchyPath.$config['page_extension'];
	  	} else {	  		
		  	$link= $config['root_url']."/index.php/".$entry->mHierarchyPath.$config['page_extension'];
		  }
	  }	else {
...
return $result;
	...
	function GenerateConfig($frontend=false, $templateid="", $languageid="") {
		$result="";
