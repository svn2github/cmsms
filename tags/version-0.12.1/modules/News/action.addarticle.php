	...
	if (isset($params['title']))
	{
		...
		if ($title != '')
		{
			if ($content != '')
			{
				...
				if ($useexp == 1)
				{
					$db->Execute($query, array($articleid, $usedcategory, $title, $content, $summary, $status, $db->DBTimeStamp($postdate), $db->DBTimeStamp($startdate), $db->DBTimeStamp($enddate), $db->DBTimeStamp(time()), $db->DBTimeStamp(time()), $userid));
				}
				else
				{
					$db->Execute($query, array($articleid, $usedcategory, $title, $content, $summary, $status, $db->DBTimeStamp($postdate), $db->DBTimeStamp($startdate), NULL, $db->DBTimeStamp(time()), $db->DBTimeStamp(time()), $userid));
				}
