	...
	if (isset($children) && count($children))
	  {
		...
		if ($content != NULL && $content->Active() && 
		    ...
		    ($content->ShowInMenu() || ($show_all == 1  && !$content->IsSystemPage())))
		  {
		    ...
		    $n = (int)($newnode->depth - $origdepth) + 1;
		    $have_nlevels = isset($params['number_of_levels']);
		    $have_depth = $have_nlevels && ($n < (int)$params['number_of_levels']);
		    $have_showparents = count($showparents) > 0;
		    $have_parent = $have_showparents && in_array($content->Hierarchy().'.',$showparents);
		    if( ($have_nlevels && $have_depth) || ($have_showparents && $have_parent) || (!$have_nlevels && !$have_showparents) )
		      {
