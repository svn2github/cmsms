...
if (!isset($params["path"])) {
	$this->Redirect($id, 'defaultadmin',$returnid,array("module_error"=>"You do not have advanced access, so you cannot upload to that path!"));
	exit;
}
		...
		switch (strtolower($extension)) {
			case "zip" : {
				...
				if ($unzipping->debugstrings!="") {
					...
					$messages.=$_FILES[$id."file_".$i]["name"]." ".$this->Lang("unpacksuccess")."</li>\n";
				}
				break;
			}
			case "tgz" :
