...
?>
		...
		function CMSmsCustomCleanup(type, value) {
			...
			switch (type) {
				...
				<?php
				if( $tiny->GetPreference('replace_cms_selflink') )
				{
					...
					echo "value = value.replace(/<a class=\"tinyreplace\" href=\"[^\"]*index\.php\?page=([0-9]+)\"[^>]*>(.*?)<\/a>/g,'<a href=\"index.php?page=$1\" class=\"tinyreplace\">$2</a>');\n";

					foreach ($content_array as $one) {
						if ($one->FriendlyName() == 'Separator') {
							continue;
						}
						$fstr = "/<a href=\"[^\"]*index\.php\?page=";
						$fstr = $fstr . $one->Id() . "[^>]*? class=\"tinyreplace\">";
						$fstr = $fstr . str_replace($search, $replace, $one->Name());
						$fstr = $fstr . "<\/a>/g";
						$rstr = "\"{cms_selflink page='" . $one->Id() . "'}\"";
						echo "value = value.replace($fstr,$rstr);\n";
					}
					echo "value = value.replace(/<a href=\"[^\"]*index\.php\?page=([0-9]+)\"[^>]* class=\"tinyreplace\">(.*?)<\/a>/g,\"{cms_selflink page='$1' text='$2'}\");";
					echo "break;\n";
					echo "case \"insert_to_editor\":\n";
					//$content_array = ContentManager::GetAllContent(false);
					$content_ops=$gCms->GetContentOperations();
					$content_array=$content_ops->GetAllContent();
					...
					foreach ($content_array as $one) {
						...
						$fstr = "/\{cms_selflink page='" . $one->Id() . "'\}/g";
						$rstr = "'<a href=\"index.php?page=" . $one->Id() . "\" class=\"tinyreplace\">";
						$rstr = $rstr . str_replace("'","\\'",$one->Name()) . "<\/a>'";
						echo "value = value.replace($fstr,$rstr);\n";
					}
					echo "value = value.replace(/\{cms_selflink page='([0-9]+?)' text='([^']+?)'\}/g,'<a href=\"index.php?page=$1\" class=\"tinyreplace\">$2<\/a>');\n";
				}
				?>
				break;
			}
...
//				$css = $gCms->variables['tinymce_templateid'];        
  			...
  			echo '
      content_css : "' . $css . "\",\n";
  			$tiny->RemovePreference("live_templateid");
			}
      ...
      themes : "advanced",
			...
			dialog_type: "modal",
			apply_source_formatting : "<?php if($tiny->GetPreference('source_formatting',1) == 1) { echo 'true'; } else { echo 'false'; } ?>",
			<?php if($tiny->GetPreference('show_path', 1) == 1) {
				echo "
			...
			if ($tiny->GetPreference("editor_width_auto",1)!=1) {
				echo "
			width : '".$tiny->GetPreference("editor_width").$tiny->GetPreference("editor_width_unit")."',";
			}
			if ($tiny->GetPreference("editor_height_auto",1)!=1) {
				echo "
			height : '".$tiny->GetPreference("editor_height").$tiny->GetPreference("editor_width_height")."',";
			}
			?>
			<?php
			...
			plugin_simplebrowser_browselinkurl : '<?php echo $basepath ?>plugins/simplebrowser/browser.html?Connector=connectors/php/connector.php',
			plugin_simplebrowser_browseimageurl : '<?php echo $basepath ?>plugins/simplebrowser/browser.html?type=Image&Connector=connectors/php/connector.php',
			plugin_simplebrowser_browseflashurl : '<?php echo $basepath ?>plugins/simplebrowser/browser.html?Type=Flash&Connector=connectors/php/connector.php',
			...
			?>
			<?php 
			$table_styles = trim($tiny->GetPreference('table_styles'));
			if( $table_styles != "" ) {
				echo "
			...
			?>
			<?php 
			$table_row_styles = trim($tiny->GetPreference('table_row_styles'));
			if( $table_row_styles != "" ) {
				echo "
			table_row_styles : \"$table_row_styles\",\n";
			}
			?>
			<?php $table_cell_styles = trim($tiny->GetPreference('table_cell_styles'));
			if( $table_cell_styles != "" ) {
				echo "
  		...
  		<?php
  		if ($tiny->GetPreference("onlyxhtmlelements",1)!=1) {
  		?>
  		invalid_elements : "",
			valid_elements : "*[*]",
