...
if (!isset($gCms)) exit;
		...
		while ($dbresult && $row = $dbresult->FetchRow())
		{
			...
			$onerow->link = $this->CreateLink($id, 'detail', $detailpage!=''?$detailpage:$returnid, '', $sendtodetail,'', true, false,'',true, 'news/'.$row['news_id'].'/'.($detailpage!=''?$detailpage:$returnid));
			$onerow->titlelink = $this->CreateLink($id, 'detail', $detailpage!=''?$detailpage:$returnid, $row['news_title'], $sendtodetail,'',false,false,'',true,'news/'.$row['news_id'].'/'.($detailpage!=''?$detailpage:$returnid));
			$onerow->morelink = $this->CreateLink($id, 'detail', $detailpage!=''?$detailpage:$returnid, $moretext, $sendtodetail,'',false,false,'',true,'news/'.$row['news_id'].'/'.($detailpage!=''?$detailpage:$returnid));
