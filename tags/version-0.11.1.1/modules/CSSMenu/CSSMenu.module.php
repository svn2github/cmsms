...
#
#$Id: CSSMenu.module.php 2114 2005-11-04 21:51:13Z wishy $

include(dirname(__FILE__)."/strripos.php");  // Required for strripos() in PHP 4 and below
...
class CSSMenu extends CMSModule
{
  ...
  {
    return "
	  1.2.2: Both vertical and horizontal menus now have the same id in the top <ul> tag. horizontal=\"1\" is now therefore useless. Horizontal or vertical is only determined by what stylesheet that is used. Also new stylesheet for horizontal version.<br />
        ...
        foreach ($allcontent as $onecontent)
        {
           ...
           for ($i = $depth; $i > $last_level; $i--)
           {
            if ($depth > 1)
            {
             // Replace the last <li> with <li class="menuparent"> so that the parent li has the class 'menuparent'. This allows the stylesheet to work.
				...
				$tmppos2 = strripos($menu, '<li class="menuactive">');

				if (!($tmppos === FALSE || ($tmppos2 !== FALSE && $tmppos !== FALSE && $tmppos2 > $tmppos)))
				{
					$tmppos2 = FALSE;
				}
				...
				if ($tmppos2 !== FALSE)
				{
					$menu = substr_replace($menu, '<li class="menuactive menuparent">', $tmppos2, 23);
				}
                else if ($tmppos !== FALSE)
                {
					$target = @ContentManager::CreateUnfriendlyHierarchyPosition($onecontent->Hierarchy());
					$target = substr($target, 0, strlen($target) - 6);

					if (strstr($gCms->variables["pageinfo"]->content_hierarchy, $target) == $gCms->variables["pageinfo"]->content_hierarchy && $gCms->variables["pageinfo"]->content_hierarchy != $target)
					{
						$menu = substr_replace($menu, '<li class="menuactive menuparent">', $tmppos, 4);
					}
					else
					{
                ...
                /*else // look for <li class="sectionheader">
                {
                  ...
                  if ($tmppos !== FALSE)
                  {
					if (strstr($gCms->variables["pageinfo"]->content_hierarchy, $onecontent->Hierarchy()) == $gCms->variables["pageinfo"]->content_hierarchy)
					{
						$menu = substr_replace($menu, '<li class="menuactive menuparent">', $tmppos, 4);
					}
					else
          ...
          { 
			...
			if (isset($gCms->variables['content_id']))
			{
				if ($onecontent->Id() == $gCms->variables['content_id'])
				{
					$menu .= " class=\"menuactive\"";
				}
				else if (strstr(@ContentManager::CreateFriendlyHierarchyPosition($gCms->variables["pageinfo"]->content_hierarchy), $onecontent->Hierarchy()) == @ContentManager::CreateFriendlyHierarchyPosition($gCms->variables["pageinfo"]->content_hierarchy))
				{
					$menu .= " class=\"menuactive menuparent\"";
				}
			}
