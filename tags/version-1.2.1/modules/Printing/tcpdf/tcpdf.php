	...
	class TCPDF {
      ...
      if ($tmp_img[2] == '1') {
        if (!function_exists("imagecreatefromgif")) {
			  	...
			  	$w = $tmp_img[0] / ($this->imgscale * $this->k);
		    	$h = $tmp_img[1] / ($this->imgscale * $this->k);
			
          $this->img_rb_x = $x + $w ;
			    $this->img_rb_y = $y + $h;
          return;
        }        
        $tmp_filename = tempnam($config["root_path"]."tmp/", 'gif4tcpdf'.md5(uniqid(rand())) );
