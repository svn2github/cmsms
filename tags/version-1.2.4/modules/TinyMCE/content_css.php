...
if (isset($config['old_stylesheet']) && $config['old_stylesheet'] == false)
{
	...
	if ($config['dbms'] == 'mysqli' || $config['dbms'] == 'mysql')
	{
		...
		if ($name != '')
			...
			$sql="SELECT c.css_text, c.css_id, c.css_name FROM ".$config['db_prefix']."css c,".$config['db_prefix']."css_assoc ac
       			WHERE ac.assoc_type='template' AND ac.assoc_to_id = ". mysql_real_escape_string($templateid, $db) ."
        		AND ac.assoc_css_id = c.css_id AND c.media_type = '" . mysql_real_escape_string($mediatype, $db) . "'
...
//		echo $sql;
		...
		$bodycss="";
		$sql="SELECT sitepref_value FROM ".$config['db_prefix']."siteprefs WHERE sitepref_name = 'TinyMCE_mapi_pref_bodycss'";
		$result=mysql_query($sql);
		if( $result && $row = mysql_fetch_assoc($result) ) {
			$bodycss = $row['sitepref_value'];
		}

		$stripbackground="";
		$sql="SELECT sitepref_value FROM ".$config['db_prefix']."siteprefs WHERE sitepref_name = 'TinyMCE_mapi_pref_striptags'";
		$result=mysql_query($sql);
		if( $result && $row = mysql_fetch_assoc($result) ) {
			$stripbackground = $row['sitepref_value'];
		}
	}
	else
	{
		...
		if ($name != '')
			...
			$sql="SELECT c.css_text, c.css_id, c.css_name FROM ".$config['db_prefix']."css c,".$config['db_prefix']."css_assoc ac
						WHERE ac.assoc_type='template' AND ac.assoc_to_id = ". pg_escape_string($templateid) ."
						AND ac.assoc_css_id = c.css_id AND c.media_type = '" . pg_escape_string($mediatype) . "'
						ORDER BY ac.create_date";
		...
		$sql="SELECT sitepref_value FROM ".$config['db_prefix']."siteprefs WHERE sitepref_name = 'TinyMCE_mapi_pref_bodycss'";
		$result=pg_query($sql);
		if( $result && $row = pg_fetch_array($result, null, PGSQL_ASSOC ) ) {
			$bodycss = $row['sitepref_value'];
		}
...
else
{
	...
	$bodycss = $gCms->modules["TinyMCE"]['object']->GetPreference("bodycss");
	$stripbackground = $gCms->modules["TinyMCE"]['object']->GetPreference("striptags");
...
$bodycss = trim(strtoupper($bodycss));
