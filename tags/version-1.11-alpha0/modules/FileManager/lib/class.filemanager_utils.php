...
final class filemanager_utils
{
  ...
  public static function get_file_list($path = '')
  {
    ...
    // convert teh cwd into a real path... slightly different for advanced mode.
    if( !$advancedmode ) {
      $realpath = self::join_path($config['root_path'],$path);
    }
    else {
	...
	$info['url']=self::join_path($config['root_url'], self::join_path($path,$file));
  ...
  public static function get_dirlist()
  {
    ...
    $advancedmode = $mod->GetPreference('advancedmode',0);
    if( $mod->AdvancedAccessAllowed() && $advancedmode )
      {
