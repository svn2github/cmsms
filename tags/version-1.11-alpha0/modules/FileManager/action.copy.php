...
if( isset($params['submit']) ) {
  ...
  if( count($errors) == 0 ) {
    $destloc = filemanager_utils::join_path($config['root_path'],$destdir);
  ...
  if( count($errors) == 0 ) {
    ...
    foreach( $selall as $file ) {
      ...
      if( $destname ) {
	$dest = filemanager_utils::join_path($config['root_path'],$destdir,$destname);
      }
      else {
	$dest = filemanager_utils::join_path($config['root_path'],$destdir,$file);
      }
	...
	$dest_thumb = filemanager_utils::join_path($config['root_path'],$destdir,$tmp);
	if( $destname ) {
	  $dest_thumb = filemanager_utils::join_path($config['root_path'],$destdir,'thumb_'.$destname);
	}
