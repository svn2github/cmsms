	...
	<div class="dashboard-box{if $smarty.foreach.box.index % 3 == 2} last{/if}">
		<nav class="dashboard-inner cf">
			...
			{foreach from=$node.children item='one'}
			 	<li><a href="{$one.url}"{if isset($one.target)} target="{$one.target}"{/if} {if substr($one.url,0,6) == 'logout' and isset($is_sitedown)}onclick="return confirm('{'maintenance_warning'|lang}')"{/if}>{$one.title}</a></li>
			{/foreach} 
			</ul>
			{/if}
		</nav>
