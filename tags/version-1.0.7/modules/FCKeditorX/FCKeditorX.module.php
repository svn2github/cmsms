...
class FCKeditorX extends CMSModule
{
	...
	function GetAuthor()
	{
		return 'Roberto Tagliento/Morten Poulsen';
	}


	function GetAuthorEmail()
	{
		return 'megabob3@yahoo.it/morten@poulsen.org';
	}
...
*/
	...
	function Upgrade($oldversion, $newversion)
	{
		...
		//Remove custom table used in earlier versions
		global $gCms;
		$db =& $gCms->GetDb();
		$dict = NewDataDictionary( $db );
	...
	function Uninstall()
	{
		...
		$this->RemovePreference('FCKConfig.CustomToolbar');
		$this->RemovePreference('FCKConfig.Styles');
	...
	function WYSIWYGTextarea($name='textarea',$columns='80',$rows='15',$encoding='',$content='',$stylesheet='', $params=array()) {
		...
		if ($ok) {
			...
			$oFCKeditor->Config['SourcePopup']='false';
	...
	{
		return 'var listitems=document.getElementsByTagName(\'iframe\');
...
*/
		$styles=$this->GetPreference("FCKConfig.CustomToolbar");
...
*/
		$styles=$this->GetPreference("FCKConfig.Styles");
	...
	function DisplaySettings($id, &$params, $returnid, $message=''){
		...
		$importallstyles = intval($this->GetPreference("FCKConfig.ImportAllStyles"));
		...
		while ($entry = $d->read()) {
			if ($entry[0]!=".") {
				if (is_dir(dirname(__FILE__).'/FCKeditor/editor/skins/'.$entry )) {
		...
		$this->smarty->assign('logoimg', '<img src="'.$this->cms->config["root_url"].'/modules/FCKeditorX/FCKeditor/editor/dialog/fck_about/logo_fckeditor.gif" width="236" height="41">');
		...
		if ($lang == 1)
		$val_lang = $lang;
		$this->smarty->assign('lang_input', $this->CreateInputCheckbox($id, "use_cms_lang", $lang, $val_lang, ''));
		...
		$this->smarty->assign('importallstyles_text', $this->Lang('importallstyles_text'));
		$this->smarty->assign('importallstyles_input', $this->CreateInputCheckbox($id, "importallstyles", "1", $importallstyles));
		$this->smarty->assign('importallstyles_warning', $this->Lang('importallstyles_warning'));

		$this->smarty->assign('xhtml_text', $this->Lang('xhtml_text'));
		$val_xhtml = -1;
		if ($xhtml == 1) $val_xhtml = $xhtml;
		...
		$this->smarty->assign('submit', $this->CreateInputSubmit($id, "submit", $this->Lang("update")));
		$this->smarty->assign('testarea', $this->CreateTextArea(true,$id,"This is a testing area","testarea",'','','','','60','8','FCKeditorX'));
	...
	function UpdateToolbar(&$params){
		$this->SetPreference('FCKConfig.CustomToolbar',$params["toolbar"]);
		$this->ShowMessage($this->Lang("toolbar_updated"));

	}

	function UpdateStyle(&$params){
		$this->SetPreference('FCKConfig.Styles',$params["styles"]);
		$this->ShowMessage($this->Lang("styles_updated"));
	}
 ...
 function SavePref(&$params){
		...
		if (isset($params['use_cms_lang']))
		$this->SetPreference('FCKConfig.CMSMSLanguage', 1);
		else
		$this->SetPreference('FCKConfig.CMSMSLanguage', 0);

		if (isset($params['importallstyles']))
		$this->SetPreference('FCKConfig.ImportAllStyles', 1);
		else
		$this->SetPreference('FCKConfig.ImportAllStyles', 0);
		...
		$this->ShowMessage($this->Lang("setting_updated"));
