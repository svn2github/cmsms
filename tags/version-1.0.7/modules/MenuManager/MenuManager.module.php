	...
	if (isset($children) && count($children))
	  {
		...
		$includeit = (($includeprefix == '') || ($includeprefix != '' && strstr($content->Alias(),$includeprefix) !== FALSE ));
		$excludeit = ($excludeprefix != '' && strstr($content->Alias(),$excludeprefix) !== FALSE );

//  		echo "DEBUG: node = ".$content->Alias();
//  		if( $includeit ) { echo " true"; } else { echo " false"; }
//  		if( $excludeit ) { echo " true"; } else { echo " false"; }
//  		echo "\n";

		if ($content != NULL && $content->Active() && 
		    ($includeit && !$excludeit) &&
		    ($content->ShowInMenu() || ($show_all == 1)))
		  {
		    $newnode =& $this->FillNode($content, $onechild, $nodelist, 
						$gCms, $count, $prevdepth, $origdepth);
