...
if(!class_exists('TCPDF')) {
	 ...
	 */ 
	define('PDF_PRODUCER','TCPDF 3.3.0.008_PHP4 (http://www.tcpdf.org)');
	...
	Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
		...
		*/
		function utf8Bidi($ta, $forcertl=false) {
			...
			if ($arabic) {
				$endedletter = array("1571","1575","1570","1573","1583","1584","1585","1586","1608","1572","1577");
				$chardata2 = $chardata;
				for ($i=0; $i < $numchars; $i++) {
					if ($unicode[$chardata[$i]['char']] == 'AL') {
						if (($i > 0) AND (($i+1) < $numchars) AND
							...
							// medial
							if (in_array($chardata[$i-1]['char'], $endedletter)) {
								if (isset($unicode_arlet[$chardata[$i]['char']][2])) {
									$chardata2[$i]['char'] = $unicode_arlet[$chardata[$i]['char']][2];
								}
							}else{
								if (isset($unicode_arlet[$chardata[$i]['char']][3])) {
									$chardata2[$i]['char'] = $unicode_arlet[$chardata[$i]['char']][3];
								}
							}
							...
							// final
							if (in_array($chardata[$i-1]['char'], $endedletter)) {
								if (isset($unicode_arlet[$chardata[$i]['char']][0])) {
									$chardata2[$i]['char'] = $unicode_arlet[$chardata[$i]['char']][0];
								}
							}else{
								if (isset($unicode_arlet[$chardata[$i]['char']][1])) {
									$chardata2[$i]['char'] = $unicode_arlet[$chardata[$i]['char']][1];
								}
							}
