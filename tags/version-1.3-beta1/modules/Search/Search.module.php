...
class Search extends CMSModule
{
  ...
  function GetSearchHtmlTemplate()
  {
    ...
    <label for="{$search_actionid}searchinput">{$searchtext}</label>:&nbsp;<input type="text" name="{$search_actionid}searchinput" size="20" maxlength="50" {$hogan}/><input name="submit" value="{$submittext}" type="submit" />
	...
	switch ($eventname)
	  ...
	  case 'ContentEditPost':
	    $content =& $params['content'];
            ...
            $db =& $this->GetDb();
            $q = "SELECT * FROM ".cms_db_prefix()."module_search_items WHERE
	  ...
	  case 'EditTemplatePost':
		...
		if( $template->active != false )
		{
			...
			$q = "SELECT * FROM ".cms_db_prefix()."module_search_items WHERE content_id = ?
