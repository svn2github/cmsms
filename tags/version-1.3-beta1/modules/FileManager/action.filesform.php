				...
				if($params["targetdir"]!='-') {
					...
					if ($params["selectedaction"]=="moveselected") {
						if(@rename($fullname,$newpathfullname)) {
							$messages.= $this->Slashes($filename)." ".$this->Lang('movedto')." ".str_replace($config['root_path'],'',$this->Slashes($newpathfullname)).'<br/>';
						} else {
							$errors.=$this->Slashes($filename)." ".$this->Lang('couldnotmove').'<br/>';
						}
					} else {
						if(@copy($fullname,$newpathfullname)) {
							$messages.=$this->Slashes( $filename)." ".$this->Lang('copiedto')." ".str_replace($config['root_path'],'',$this->Slashes($newpathfullname))."<br/>";
						} else {
							$errors.=$this->Slashes($filename)." ".$this->Lang('couldnotcopy').'<br/>';
						}
					}
				}
				//$errors.=$fullname."<br/>";
