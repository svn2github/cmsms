...
if (isset($params["makerssbutton"]))
  {
    $category = '';
    $url = 'news/rss';
    if( isset($params['rsscategory']) )
      {
	$category = trim($params['rsscategory']);
	$params['category'] = $params['rsscategory'];
	unset($params['rsscategory']);
	$url .= '/'.$category;
      }
    $params["showtemplate"] = "false";
    echo $this->CreateLink($id, 'rss', $returnid, "<img border=\"0\" src=\"images/cms/rss.png\" alt=\"RSS Newsfeed\" />", $params,'',false,false,'',true,$url);
    return;
  }
