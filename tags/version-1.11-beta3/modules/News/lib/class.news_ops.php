...
final class news_ops
{
  ...
  public static function get_categories($id,$params,$returnid=-1)
  {
	...
	for( $i = 0; $i < count($tmp); $i++ )
	  {
	    if( !count($categories) || in_array($tmp[$i]['category_name'],$categories) )
	      {
		$catinfo[] = $tmp[$i];
	      }
	  }
  ...
  static public function &get_article_by_id($article_id,$for_display = TRUE)
  {
    ...
    $query = 'SELECT mn.*, mnc.news_category_name FROM '.cms_db_prefix().'module_news mn 
              ...
              AND (('.$db->ifNull('end_time',$db->DbTimeStamp(1)).' = '.$db->DbTimeStamp(1).') OR (end_time > NOW()))';
  ...
  public static function get_fields($news_id,$public_only = true)
  {
    if( $news_id <= 0 ) return;
