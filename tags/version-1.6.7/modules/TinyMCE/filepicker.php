...
$fmallowed=false;
$showfm=false;
if ($tiny->CheckPermission("Modify Files")) {
  $fmallowed=true;
  if ($tiny->CheckPermission('allowadvancedprofile')) {
  ...
  if ($file["isdir"]) {
    ...
    if (IsDirEmpty($tiny->Slash($thisdir,$file["name"]))) {
      if ($usefm) {
        ...
        $onerow->deletelink.="><img style='border:0px' src='".$config["root_url"]."/".$config["admin_dir"]."/themes/default/images/icons/system/delete.gif' alttext='".$tiny->Lang("deletedir")."' />";
        $onerow->deletelink.="</a>\n";
      }
    }
    $dircount++;
    ...
    if ($usefm) {
      ...
      $onerow->deletelink.="><img style='border:0px' src='".$config["root_url"]."/".$config["admin_dir"]."/themes/default/images/icons/system/delete.gif' alttext='".$tiny->Lang("deletefile")."' />";
      $onerow->deletelink.="</a>\n";
    }
