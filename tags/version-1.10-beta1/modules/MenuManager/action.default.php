	  ...
	  if( $parent )
	    {
		  ...
		  if( $obj->Active() && ($obj->ShowInMenu() || (isset($params['show_all']) && $params['show_all'])) )
		  {
		    $rootnode[] = $onechild;
		  }
	        }
			...
			if (isset($content))
			{
				...
				else
				{
					...
					if ($content->Active() && ($content->ShowInMenu() || ($params['show_all'] == 1 && !$content->IsSystemPage())))
					{
						...
						$this->FillNode($content, $rootnode, $nodelist, $gCms, $count, $origdepth, $prevdepth, $deep, $params);
		...
		if (count($items) > 0)
		{
			...
			while (list($key) = each($items))
			{
				...
				if ($curnode)
				{
					...
					{
						$prevdepth = 1;
						$newnode =& $this->FillNode($curcontent, $curnode, $nodelist, $gCms, $count, $prevdepth, 1, $deep);
						//$newnode->depth = 1;
						//$newnode->prevdepth = 1;
					}
				}
		...
		foreach( $rootnode as $onenode )
		  {
		    ...
		    if( $content )
		      {
			$this->FillNode($content, $onenode, $nodelist, $gCms, $count, $prevdepth, $prevdepth, $deep);
			if( !isset($params['number_of_levels']) || ($params['number_of_levels'] > 1) )
			  {
			    // we are getting more than one level.
			    $this->GetChildNodes($onenode,$nodelist,$gCms,$prevdepth,$count,$params,
