...
Insert Smarty {literal} around selection|{literal}|{/literal}");
			...
			$this->SetPreference('cmslinkerstyle',"a");
...
";
	...
	function AddEntry($menu,$entry) {
		...
		if ($this->GetPreference("cmslinkerstyle","a")=="a") {
...
tinymce.PluginManager.add('cmslinker', tinymce.plugins.CMSLinkerPlugin);
	...
	function GenerateConfig($frontend=false, $templateid="", $languageid="") {
		...
		if( isset($_SESSION['tiny_live_textareas']) ) $this->smarty->assign("textareas",$_SESSION["tiny_live_textareas"]);
