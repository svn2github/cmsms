...
function toggle_by_name(name)
{
  var els = document.getElementsByName(name);
  if( els.length == 0 ) return;
  var state = els[0].style.display;
...
{/if}
	<div class="pageoverflow">
		...
		<p class="pageinput"><input type="checkbox" name="{$actionid}useexp" {if $useexp == 1}checked="checked"{/if} onclick="toggle_by_name('expiryinfo');" class="pagecheckbox"/></p>
	</div>
	<div class="pageoverflow" name="expiryinfo" {if $useexp != 1}style="display: none;"{/if}>
