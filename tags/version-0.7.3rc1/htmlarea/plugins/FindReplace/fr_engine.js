...
clearMarks();
  ...
  var message = i18n["Done"]+':\n\n';
    if(matches > 0) {
      if(matches == 1) message += matches+' '+i18n["found item"];
      else             message += matches+' '+i18n["found items"];
      if(replaces > 0) {
        if(replaces == 1) message += ',\n'+replaces+' '+i18n["replaced item"];
        else              message += ',\n'+replaces+' '+i18n["replaced items"];
      }
    hiliteAll();
    disab('fr_hiliteall',false);
    } else { message += '"'+pattern+'" '+i18n["not found"]; }
  alert(message+'.');
  }
};
